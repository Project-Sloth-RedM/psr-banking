...
--[[ -- Only used by the following "psr-banking:initiateTransfer"
...
RegisterNetEvent('psr-banking:initiateTransfer', function(_)
    --[[
    local _src = source
    local _startChar = PSRCore.Functions.GetPlayer(_src)
    ...
    if (checkAccount) then
        ...
        if receiptName ~= false or receiptName ~= nil then
            local userOnline = exports.psr-base:checkOnline(cid)

            if userOnline ~= false then
                -- User is online so we can do a straght transfer
                local _targetUser = exports.psr-base:Source(userOnline)
                if acType == "Current" then
                    ...
                    TriggerClientEvent('psr-banking:openBankScreen', _src)
                    TriggerClientEvent('psr-banking:successAlert', _src, 'You have sent a bank transfer to '..receiptName..' for the amount of $'..data.amount)
                else
        ...
        TriggerClientEvent('psr-banking:transferError', _src, 'The account details entered could not be located.')
    end
]]
end)
...
PSRCore.Functions.CreateCallback('psr-banking:getBankingInformation', function(source, cb)
    local xPlayer = PSRCore.Functions.GetPlayer(source)
...
RegisterNetEvent('psr-banking:createBankCard', function(pin)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    local cid = xPlayer.PlayerData.citizenid
    ...
    TriggerClientEvent('psr-banking:openBankScreen', src)
    TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.debit_card'))

    TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** successfully ordered a debit card")
end)

RegisterNetEvent('psr-banking:doQuickDeposit', function(amount)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    ...
    if tonumber(amount) <= currentCash then
        ...
        if bank then
            TriggerClientEvent('psr-banking:openBankScreen', src)
            TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.cash_deposit', {value = amount}))
            TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash deposit of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('psr-banking:toggleCard', function(toggle)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
...
RegisterNetEvent('psr-banking:doQuickWithdraw', function(amount, _)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentCash then
        ...
        if cash then
            TriggerClientEvent('psr-banking:openBankScreen', src)
            TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.cash_withdrawal', {value = amount}))
            TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a cash withdrawal of $"..amount.." successfully.")
        end
    end
end)

RegisterNetEvent('psr-banking:updatePin', function(currentBankCard, newPin)
    if newPin ~= nil then
        local src = source
        local xPlayer = PSRCore.Functions.GetPlayer(src)
        while xPlayer == nil do Wait(0) end
        ...
        MySQL.update('UPDATE bank_cards SET cardPin = ? WHERE record_id = ?', {
            ...
            if result == 1 then
                TriggerClientEvent('psr-banking:openBankScreen', src)
                TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.updated_pin'))
            else
                TriggerClientEvent('PSRCore:Notify', src, 'Error updating pin', "error")
            end
        end)
    end
end)

RegisterNetEvent('psr-banking:savingsDeposit', function(amount)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentBank then
        ...
        TriggerClientEvent('psr-banking:openBankScreen', src)
        TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.savings_deposit', {value = tostring(amount)}))
        TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', 'lightgreen', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings deposit of $"..tostring(amount).." successfully..")
    end
end)

RegisterNetEvent('psr-banking:savingsWithdraw', function(amount)
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    while xPlayer == nil do Wait(0) end
    ...
    if tonumber(amount) <= currentSavings then
        ...
        while savings == nil do Wait(0) end
        TriggerClientEvent('psr-banking:openBankScreen', src)
        TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.savings_withdrawal', {value = tostring(amount)}))
        TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', 'red', "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** made a savings withdrawal of $"..tostring(amount).." successfully.")
    end
end)

RegisterNetEvent('psr-banking:createSavingsAccount', function()
    local src = source
    local xPlayer = PSRCore.Functions.GetPlayer(src)
    local success = createSavingsAccount(xPlayer.PlayerData.citizenid)
    repeat Wait(0) until success ~= nil
    TriggerClientEvent('psr-banking:openBankScreen', src)
    TriggerClientEvent('psr-banking:successAlert', src, Lang:t('success.opened_savings'))
    TriggerEvent('psr-log:server:CreateLog', 'banking', 'Banking', "lightgreen", "**"..GetPlayerName(xPlayer.PlayerData.source) .. " (citizenid: "..xPlayer.PlayerData.citizenid.." | id: "..xPlayer.PlayerData.source..")** opened a savings account")
end)


PSRCore.Commands.Add('givecash', Lang:t('command.givecash'), {{name = 'id', help = 'Player ID'}, {name = 'amount', help = 'Amount'}}, true, function(source, args)
  local src = source
	...
	if id and amount then
		local xPlayer = PSRCore.Functions.GetPlayer(src)
		local xReciv = PSRCore.Functions.GetPlayer(id)
                    ...
                    if amount > 0 then
                        if xPlayer.Functions.RemoveMoney('cash', amount) then
                            if xReciv.Functions.AddMoney('cash', amount) then
                                TriggerClientEvent('PSRCore:Notify', src, Lang:t('success.give_cash',{id = tostring(id), cash = tostring(amount)}), "success")
                                TriggerClientEvent('PSRCore:Notify', id, Lang:t('success.received_cash',{id = tostring(src), cash = tostring(amount)}), "success")
                                TriggerClientEvent("payanimation", src)
                            else
                                -- Return player cash
                                xPlayer.Functions.AddMoney('cash', amount)
                                TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.not_give'), "error")
                            end
                        else
                            TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.not_enough'), "error")
                        end
                    else
                        TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.invalid_amount'), "error")
                    end
				else
					TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.too_far_away'), "error")
				end
			else
				TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.dead'), "error")
			end
		else
			TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.wrong_id'), "error")
		end
	else
		TriggerClientEvent('PSRCore:Notify', src, Lang:t('error.givecash'), "error")
	end
end)
