        ...
        if not self.cardActive then
            ...
            MySQL.update('UPDATE bank_cards SET cardnumber = ?, cardPin = ?, cardDecrypted = ?, cardActive = ?, cardLocked = ?, cardType = ? WHERE citizenid = ? AND record_id = ?', {
                ...
                if self.source ~= -1 then
                    TriggerClientEvent('psr-banking:client:newCardSuccess', self.source, cardNumber, friendlyName)
                    local xPlayer = PSRCore.Functions.GetPlayer(self.source)
...
RegisterNetEvent('psr-banking:server:registerCurrentAccount', function(cid)
...
RegisterNetEvent('psr-banking:server:registerSavingsAccount', function(cid)
